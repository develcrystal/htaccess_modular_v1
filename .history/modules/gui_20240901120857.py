from tkinter import Tk, BooleanVar, Checkbutton, ttk, Text, Scrollbar, VERTICAL, RIGHT, Y, END, simpledialog, messagebox
...
class HTAccessOptimizerGUI:
    def __init__(self, root, start_process_callback, stop_process_callback, pause_process_callback, toggle_debug_mode_callback):
        ...
        # Colors and Tags for Log Output
        ...
        self.legend_frame.pack(pady=5, padx=10, fill="x")
        ttk.Label(self.legend_frame, text="Legende:", font=("Helvetica", 10, "bold")).pack(side="left")
        ttk.Label(self.legend_frame, text="Erfolgreich (OK)", background="green", foreground="white").pack(side="left", padx=5)
        ttk.Label(self.legend_frame, text="Redirect", background="yellow", foreground="darkorange").pack(side="left", padx=5)
        ttk.Label(self.legend_frame, text="Fehler (404/Andere)", background="lightcoral", foreground="red").pack(side="left", padx=5)
    ...
    def open_excel_report(self):
        # Function to open the Excel file (set in main.py)
        pass

    def open_output_folder(self):
        # Function to open the output folder (set in main.py)
        pass
    ...
    def ask_for_sitemap_url(self):
        sitemap_url = simpledialog.askstring("Sitemap-URL", "Bitte geben Sie die URL Ihrer Sitemap ein:")
        return sitemap_url

    def ask_for_base_url(self):
        base_url = simpledialog.askstring("Basis-URL", "Bitte geben Sie die Basis-URL Ihrer Website ein:")
