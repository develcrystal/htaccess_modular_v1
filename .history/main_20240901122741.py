...
def start_process(use_threads, htaccess_path, excel_path, sitemap_url, base_url):
    ...
    if sitemap_url:
        # Zeige dem Benutzer eine Statusnachricht, dass die Sitemap verarbeitet wird
        gui.set_status("Verarbeite Sitemap...")
        
        # Sitemap-Verarbeitung in den Hintergrund-Thread verschieben
        sitemap_urls = list(parse_sitemap(sitemap_url))  # URLs aus Sitemap
        urls_to_test.extend(sitemap_urls)

    # Zeige dem Benutzer eine Statusnachricht, dass die URLs getestet werden
    gui.set_status("Teste URLs...")
